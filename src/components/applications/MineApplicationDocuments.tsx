...
export async function MineApplicationDocuments({ applicationId }: { applicationId: string }) {
  ...
  return <section className="space-y-4 rounded-2xl border border-border bg-surface p-6"><h2 className="font-bold">Mineの応募書類</h2><p className="text-sm text-muted-foreground">応募時に共有された内容です。後からのプロフィール変更は反映されません。</p>{documents.length ? documents.map(doc => <MineDocumentSnapshot key={doc.title} document={doc} />) : <p className="text-sm">書類を共有せずに応募しました。</p>}</section>;
}
